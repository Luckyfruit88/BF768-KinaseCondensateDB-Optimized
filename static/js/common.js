...
function formatVal(k,v){if(['has_dna','has_rna','has_cmods','has_condensatopathy','reviewed_flag'].includes(k))return yes(v); if(k==='status')return v==1?'Active':'Disabled'; if(k==='role')return v==='管理员'?'Administrator':'User'; return v}
...
function renderCell(k,v){const fv=formatVal(k,v);if(k==='uniprot_accession'&&fv)return `<a href="${uniprotUrl(fv)}" target="_blank" rel="noopener">${esc(fv)}</a>`;if(k==='pmid'&&fv)return renderPmidLinks(fv);return esc(fv)}
